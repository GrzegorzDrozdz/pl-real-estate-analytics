...
# Konfiguracja strony jako pierwsza
st.set_page_config(
...
# 3. Pasek boczny - opcje filtrowania
...
with k3:
    ...
    st.metric("🏷️ Mediana ceny", f"{avg_price / 1e6:,.2f} mln zł" if pd.notna(avg_price) else "Brak")
with k4:
...
# 5. Nawigacja zakładek
tabs = st.tabs([
    "📊 Przegląd rynku",
    "📈 Trendy cenowe",
    "🏗️ Charakterystyka ofert",
...
st.markdown("---")
st.markdown("""
<div style="text-align:center; color:#475569; font-size:0.85rem; padding: 10px 0 20px 0;">
    🏠 <strong style="color:#64748b">Polski Rynek Nieruchomości 2023–2024</strong> · 
    15 miast · Sierpień 2023 – Czerwiec 2024 ·     
</div>
""", unsafe_allow_html=True)